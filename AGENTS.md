...
| Route | Role |
| --- | --- |
| `/login` | Magic link is the default (“Email me a link”). Password is behind **Use a password instead**. Invite-only; OTP never self-registers (`shouldCreateUser: false`). Household invites may create the Auth user. Supports `?next=` return after sign-in. Magic links land on `/auth/callback` (server OTP verify + cookies). |
...
| `/meals/[id]` | Counter Cook Mode (mise en place + Step-by-Step or All Steps Overview; custom timer minutes). Edit is a secondary mode on the same page. |
