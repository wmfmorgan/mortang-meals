...
Auth is invite-only **Google OAuth** (no magic-link emails). Hosted on Vercel + Supabase Postgres is supported. The browser never calls an AI provider. Open signup, shared households, and Netlify stay out of scope.
...
- Supabase Auth Google OAuth (`@supabase/ssr`), invite-only (signup off + admin-created users; Google email must match), `households.owner_id`
...
| Route | Role |
| --- | --- |
| `/login` | Google sign-in. Invite-only; admin must create the user (matching Gmail) before Continue with Google works. |
